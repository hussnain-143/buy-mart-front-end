...
const SystemLogs: React.FC<{ logs?: any[] }> = ({ logs: propLogs }) => {
    const displayLogs = propLogs && propLogs.length > 0
        ? propLogs.map(log => ({
            id: log._id,
            message: log.action,
            time: new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            description: `${log.user_id?.firstName} ${log.user_id?.lastName} (${log.user_id?.role})`
        }))
        : logs;
                ...
                {displayLogs.map((log: any) => (
                    <div key={log.id} className="relative flex flex-col group">
                        ...
                        <div className="flex justify-between items-start pl-4">
                            <h4 className="text-sm font-bold text-secondary group-hover:text-primary transition-colors line-clamp-1">
                                {log.message}
                            </h4>
                            <span className="text-[10px] font-bold text-secondary/50 bg-secondary/10 px-2 py-1 rounded-md shrink-0 ml-2">
                                {log.time}
                            </span>
                        </div>
